pipeline {
  ...
  environment {
    ...
    EKS_CLUSTER    = 'floral-monster-1761574697'
    KUBE_NAMESPACE = 'app-prod'
    // Avoid IMDS lookups during aws cli calls if no instance role is present
      ...
      steps {
        withCredentials([[$class: 'AmazonWebServicesCredentialsBinding', credentialsId: 'aws-jenkins']]) {
          sh '''
            ...
            # Update Deployment image (adjust names if yours differ)
            kubectl -n ${KUBE_NAMESPACE} set image deployment/app app=${ECR_REGISTRY}/${ECR_REPO}:${BUILD_NUMBER}

            # Wait for rollout
            kubectl -n ${KUBE_NAMESPACE} rollout status deployment/app --timeout=180s
